...
import { faThumbsUp as fasThumbsUp, faMagnifyingGlass}  from '@fortawesome/free-solid-svg-icons'
...
function Newsfeed() {
  ...
  const [sortList, setSortList] =useState([{field: "Latest", isSelected: true},
...
const [addingPost, setAddingPost] = useState(false)
  ...
  const handleScrollnFilterChange = (selected) => {
  ...
  const getInterests = () => {
    api.get(`/api/interests`)
      .then(response => {
        setInterests(response.data.interests.map(
         interest => ({ field: interest.interest, isSelected: false,  src: interest.url, id: interest.interestid})));
        })
  ...
  const getLike = () => {
    ...
    api.get('/api/getlikes', 
      { params: { username: username } }
    )
    .then(response => {
      // Assuming the response includes the newly added comment
      console.log(response.data.likes.map(like => like.postid))
      setLiked(response.data.likes.map(like => like.postid))
      console.log(liked)
    })
    .catch(error => console.error(`Error getting likes for user ${username}:`, error));
  };

  const handleLike = (postId) => {
    
    const add_operation = !liked.includes(postId);
    api.post('/api/addlikes', {
      postid: postId,
    ...
    .catch(error => console.error(`Error adding/removing likes to post ${postId}:`, error));
  };
  ...
  const getFilteredAndSortedPosts = () => {
    let filteredPosts = posts.filter(post =>
      ...
      (interests.reduce((count, item) => count + (item.isSelected ? 1 : 0), 0) === 0 || interests.map(item => {
  ...
  const addComment = (postId) => {
    const commentTimestamp = new Date().toISOString().slice(0, 19).replace('T', ' ');
    const commentObj = {
      comment: newComment,
    ...
    api.post('/api/comments', commentObj)
    .then(response => {
      // Assuming the response includes the newly added comment
      const newComment = {...commentObj, commentid: response.data};
      setComments(prevComments => ({
        ...prevComments,
  ...
  return (
    ...
    <div className='bg-slate-50'>
      {addingPost &&
        <div class="fixed top-0  left-0 z-10 w-[100%] h-[100%] bg-black bg-opacity-50 items-center justify-center" onClick={() => setAddingPost(false)}>
          <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-lg z-50 w-[30%] border-none' onClick={(e) => e.stopPropagation()}>
            <form className="form-container " onSubmit={handlePostSubmit}>
              <div>
                  <label htmlFor="interest">Category:</label>
                  <select id="interest" name="interestid" value={formData.interestid || ''} onChange={handlePostChange}>
                      <option value="" disabled>Choose a Category</option>
                      {interests.map(interest => (
                        <option key={interest.id} value={interest.id}>{interest.field}</option>
                      ))}
                  </select>
              </div>
              <div>
                  <label htmlFor="header">Header:</label>
                  <input type="text" id="header" name="header" value={formData.header} style={{ fontWeight: 'bold' }} onChange={handlePostChange} />
              </div>
              <div>
                  <label htmlFor="description">Description:</label>
                  <textarea id="description" name="description" value={formData.description} onChange={handlePostChange} />
              </div>
      ...
      <img className ='hover:cursor-pointer' onClick={() => navigate('/newsfeed')} src='/logov3.png' alt='logo' width = "30" />
      <div class='flex items-center'>
        <h3 className ='hover:cursor-pointer' onClick={() => navigate('/personal')}> {username}</h3>
        <button className="ml-3 bg-[#eb580a]  hover:bg-[#ff9f4b] text-white font-bold py-1 px-3  rounded-md " onClick={() => logout()} >
        Log out
        </button>
      </div>
      </nav>
      <div name="banner" className="flex justify-center items-center mt-12">
        <div className = 'flex flex-col w-[30%] mr-10'>
          <div className='font-bold font-montserrat text-3xl flex-wrap'>
        The people platform that meet all your needs! 
          </div>
          <div className='font-large text-lg flex-wrap'>
  Whatever your needs, from looking for a room mate to looking for a soul mate, there are hundreds of university friends that you can connect with.
    
          </div>
        </div>
        <img src="cycling.png" alt="banner" width="220"/>
      ...
      <div className = 'px-24'>
        <div className='flex flex-col mt-16'>
          ...
          <div className ='flex space-x-8 justify-between mt-16 mb-32'>
            {interests.map((interest, index) => {
              
              return (
                <div className = 'flex flex-col items-center hover:cursor-pointer' onClick={() => handleScrollnFilterChange(interest.field)}>
                <img src={interest.src} alt='logo' width = "250" className = 'rounded-full w-64 h-64 hover:opacity-70'/>
                 {/* <div key={index} className="select-none flex flex-col rounded-full bg-[#f8ae86] w-64 text-center items-center justify-center pb-5 h-64 hover:cursor-pointer" 
                  
                  // style={{ backgroundColor: interest.isSelected ? '#eb580a' : '#f8ae86' }} 
                  >
                   */}
                  
                  <span className = 'text-xl font-bold pt-5 font-montserrat text-slate-600'>{interest.field} </span>
        ...
        <div className='flex justify-between pt-12' ref={scrollRef}> 
          ...
          Posts from communities at {uni}: 
          </div>
          
          <button className="mb-8 font-bold text-[#eb580a] rounded-md border-2 border-[#eb580a] px-2 mr-8" onClick={() => setAddingPost(true)} >
          Make a post
          </button>
        </div>
        <div className='flex flex-col'>
          <div className='flex justify-center h-12 mb-5'> 
            <div className = 'flex items-center border-2 border-gray-700 rounded-3xl w-[50%] px-3 py-1'>
              <FontAwesomeIcon icon={faMagnifyingGlass} color = "#5d6269" size='1x'/> 
              <input
              type="text"
          ...
          <div className='flex space-x-3 mb-8'>
            <DropdownSingle sortList = {sortList} handleSortChange = {handleSortChange} />
            <DropdownMulti filteredList = {interests} handleFilterChange = {handleFilterChange} />
          </div>
            ...
            <div className="flex">
              ...
              <div className = 'flex space-x-3 min-h-12  mt-2'>
                {interests.map((interest, index) => {
                  ...
                  return (<div key={index} className ='rounded-lg bg-blue-200 h-6 px-2 text-center '>
                    {interest.field}
                  </div>)
                  }
                })
              ...
              {showClear && 
                <button className ='rounded-lg bg-slate-200 h-6 px-2' onClick={()=>clearFilter()}>
                  Clear
                </button>}
              </div>
          </div>
        ...
        <div className="flex flex-col">
          ...
          > 
            <div className='flex space-x-2'>
              <img src='user-icon3.png' width='25' className={`border-2 border-[${String(stringToColor(post.userid))}] rounded-full`}/>
              <h2 className='text-md font-semibold'>{post.userid}</h2>
            </div>
            <p className = 'text-2xl font-bold mb-5'> {post.header} (#{post.postid})</p>
              ...
              <div className = 'flex space-x-2'>
                <button onClick={() => handleLike(post.postid)}>
                  {liked.includes(post.postid) 
                    ? <FontAwesomeIcon id={post.postid} icon={fasThumbsUp} color = "Dodgerblue"/> 
                    : <FontAwesomeIcon id={post.postid} icon={faThumbsUp} color = "Dodgerblue"/>
                  }
                </button>
                <p>{post.number_of_likes}</p>
                
              </div>
              <button onClick={() => {toggleCommentsVisibility(post.postid) }} className = 'mr-2'>
              ...
              <div className = 'flex flex-col'>
                {comments[post.postid]?.map((comment) => (
                  <div className = 'flex pt-2 pl-5 space-x-2'>
                    <img src='user-icon3.png' width='25' height='25' className={`border-2 border-[${String(stringToColor(comment.comment_userid))}] rounded-full h-6 mt-2`}/>
                    <div key={comment.commentid} className ='py-2 px-5 mt-2 rounded-md bg-slate-200 flex-col min-w-[90%]'>
                      <div>
                        <div className='flex justify-between mb-2'>
                          <div className ='flex space-x-2'>
                            
                            <p className="font-semibold">{comment.comment_userid} </p>
                          </div>
                          <span className ='text-gray-400 text-sm'>on {new Date(comment.comment_timestamp).toLocaleDateString()} </span>
                        </div>
                      </div>
                      <p>{comment.comment}</p>
                    </div>
                ...
                {addingCommentToPostId === post.postid && (
                   <div className = 'flex pt-2 pl-5 space-x-2'>
                    <img src='user-icon3.png' width='25' height='25' className={`border-2 border-[${String(stringToColor(username))}] rounded-full h-6 mt-2`}/>
                    <div key={username} className ='py-2 px-5 mt-2 rounded-md bg-slate-200 flex-col min-w-[90%]'>
                      <div className='flex flex-col items-end pt-1 pl-1 space-x-2'>
                        <textarea className="p-2 bg-transparent border-none resize-none w-full"
                        value={newComment}
                        onChange={(e) => setNewComment(e.target.value)} placeholder="Write a comment..."></textarea>

                        
                        <button onClick={() => addComment(post.postid, newComment)}>
                          <FontAwesomeIcon id={post.postid} icon={faPaperPlane} color = "#59536a" size='1.5x'/> 
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      <footer ref= {lastPostElementRef} className='text-center border-t-gray-300 border-t-2 mt-8 mb-2 h-12 min-h-12 w-[90%] text-gray-500'>
        © ConnectU 2024 
      </footer>
      </div>
    </div>
