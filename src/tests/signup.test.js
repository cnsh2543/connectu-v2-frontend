...
describe('<SignUpForm>', () => {
  ...
  it('should render skelenton of page',async () => {
    ...
    api.get.mockImplementation((url) => {
      if (url === '/api/degreelevels') {
        return Promise.resolve({ data: {"degreeLevels" : [{ degreelevelid: 1, degreelevelname: 'Bachelor' }]}});
      } else if (url === '/api/degrees') {
        return Promise.resolve({ data: {"degrees":[{ degreeid: 1, degreename: 'ABC' }]} });
